...
log_dir=$JABAS_REMOTE_STORAGE/resnet50_jabas_convergence_log
if [ -d $log_dir ] && [ $rank == "0" ] ; then
  check_dir_and_copy_backup resnet50_jabas_convergence_log
fi
elastic_ckpt_dir=$JABAS_REMOTE_STORAGE/resnet50_elastic_convergence_ckpt
if [ -d $elastic_ckpt_dir ] && [ $rank == "0" ] ; then
  check_dir_and_copy_backup resnet50_elastic_convergence_ckpt
fi
