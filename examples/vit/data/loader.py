...
def create_iidp_loader(
    ...
    if is_adaptive_training:
        loader_class = jabas.data.AdaptiveDataLoader
    else:
        loader_class = iidp.data.DataLoader

    if use_multi_epochs_loader:
        loader_class = MultiEpochsDataLoader
        raise ValueError(f'Not support --use_multi_epochs_loader for IIDP')
            ...
            if use_prefetcher:
                ...
                if re_prob > 0.:
                    random_erasing = RandomErasing(
                            probability=prefetch_re_prob,
                            mode=re_mode,
                            max_count=re_count,
                            num_splits=re_num_splits)
                imagenet_mean = torch.tensor([x * 255 for x in mean]).cuda().view(1, 3, 1, 1)
                imagenet_std = torch.tensor([x * 255 for x in std]).cuda().view(1, 3, 1, 1)
                inputs = inputs.float().sub_(imagenet_mean).div_(imagenet_std)
                if random_erasing is not None:
                    inputs = random_erasing(inputs)
    ...
    if is_adaptive_training is True:
        loader = loader_class(
            dataset,
